...
function doComment(btn, e, commentid, spanid) {
    ...
    if (e === "edit_clicked") {
        if (!mode) {
            ...
            $.ajax({
                ...
                if (data.result) { // işlem başarılıysa
    ...
    // Silme İşlemi
        ...
        if (!dialog_res) return false;   // Hayırsa false döndürüp işlemi bitir

        $.ajax({
            method: "GET",
            url: "/Comment/Delete/" + commentid
        }).done(function (data) {                  // başarılıysa

            if (data.result) {                     // başarılı ve silindiysa
                // yorumlar partial tekrar yüklenir..
                $(modalCommentBodyId).load("/Comment/ShowNoteComments/" + noteid);       // yorumları göster
            } else {                               // silinemediyse
                alert("Yorum silinemedi.");
            }

        }).fail(function () {                     // başarısızsa
            alert("Sunucu ile bağlantı kurulamadı.");
        });
